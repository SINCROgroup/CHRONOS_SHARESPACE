...
public class ChooseL2ScreenAppState extends AbstractAppState implements ScreenController {
    private static final Logger logger = Logger.getLogger(ChooseL2ScreenAppState.class.getName());
    ...
    public ChooseL2ScreenAppState(SimpleApplication app,String name) {
    ...
    private void setUpView() {
        ...
        if (Settings.getSettings().getCount() == 1) {
            l.setText("Set the features of the 2nd L2");
        } else if (Settings.getSettings().getCount() == 2) {
            l.setText("Set the features of the 3rd L2");
        } else if (Settings.getSettings().getCount() >= 3) {
            int var10001 = Settings.getSettings().getCount();
            l.setText("Set the features of the " + (var10001 + 1) + "th L2");
        }
        ...
        SetList(this.controller,(Paths.get("src\\Administrator\\Algorithm\\L2")).toString());
        //this.controller.addItem("deep Q network");
        System.out.println((Paths.get("src\\Administrator\\Algorithm\\L2")).toAbsolutePath().toString());
    }
    ...
    public void setVP() {
        ...
        Settings.getSettings().setL2_inner_d(d, Integer.parseInt(textFieldNumPlayer.getRealText()));
        Settings.getSettings().setL2_algorithm(c,Integer.parseInt(textFieldNumPlayer.getRealText()));
        Settings.getSettings().setL2_role(r,Integer.parseInt(textFieldNumPlayer.getRealText()));
        for(int i=0;i<this.numparameter;i++){
            Settings.getSettings().setParameter(Settings.getSettings().getCount(),i,Float.parseFloat(textField[i+1].getRealText()));
        }

        Settings.getSettings().setCount(Settings.getSettings().getCount() + 1);
        if (Settings.getSettings().getNumL2() == Settings.getSettings().getCount()) {
            Settings.getSettings().setCount(0);
            logger.info("Settings of L2s is completed.");
            if(Settings.getSettings().getNumL3_a()>0) {
                this.cleanup();
                this.app.getStateManager().detach(this);
                ChooseL3_aScreenAppState vpController1 = new ChooseL3_aScreenAppState(this.app, this.name);
                this.nifty.fromXml("Interface/chooseVPAdmin.xml", "chooseVPAdmin", new ScreenController[]{vpController1});
            ...
            ChooseL2ScreenAppState vpController = new ChooseL2ScreenAppState(this.app,this.name);
    ...
    public void update(float tpf) {
        ...
        if(!Objects.equals(c, Prevc)){
            ...
            try{
                ...
                clazz = Class.forName("Administrator.Algorithm.L2."+c);
